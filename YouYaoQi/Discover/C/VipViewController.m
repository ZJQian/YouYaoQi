...
#define img [UIImage imageNamed:@"vipBg_621x470_"]


@interface VipViewController ()<UINavigationControllerDelegate,UITableViewDelegate,UITableViewDataSource>
...
- (void)viewDidLoad {
    ...
    self.navView.lb_navTitle.text = @"VIP";
    self.navView.lb_navTitle.textColor = [UIColor whiteColor];
    [CNetWorking getWithUrl:vipList params:@{@"sexType":@3} success:^(id  _Nonnull response) {
        self.dataArray = [FindComicModel mj_objectArrayWithKeyValuesArray:response[@"data"][@"returnData"][@"newVipList"]];
...
- (void)navigationController:(UINavigationController *)navigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated {
    BOOL isShow = [viewController isKindOfClass:[VipViewController class]];
